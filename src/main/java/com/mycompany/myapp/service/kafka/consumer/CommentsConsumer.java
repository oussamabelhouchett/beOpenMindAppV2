package com.mycompany.myapp.service.kafka.consumer;

import io.vavr.control.Either;
import org.apache.kafka.clients.consumer.ConsumerRecord;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;
import org.springframework.beans.factory.annotation.Value;
import org.springframework.context.annotation.Bean;
import org.springframework.core.task.SimpleAsyncTaskExecutor;
import org.springframework.stereotype.Service;
import com.mycompany.myapp.config.KafkaProperties;
import com.mycompany.myapp.domain.Comments;
import com.mycompany.myapp.service.kafka.serde.DeserializationError;

@Service
public class CommentsConsumer extends GenericConsumer<Comments> {

    private final Logger log = LoggerFactory.getLogger(CommentsConsumer.class);

    public CommentsConsumer(@Value("${kafka.topic.comments}") final String topicName, final KafkaProperties kafkaProperties) {
        super(topicName, kafkaProperties.getConsumer().get("comments"), kafkaProperties.getPollingTimeout());
    }

    @Override
    protected void handleMessage(final ConsumerRecord<String, Either<DeserializationError, Comments>> record) {

        // TODO /!\ Maybe you could delete the next log calls to avoid disclosing personal user information

        final Either<DeserializationError, Comments> value = record.value();

        if (value == null) {
            log.error("Null value in record {}", record);
            return;
        }

        if (value.isLeft()) {
            log.error("Deserialization record failure: {}", value.getLeft());
            return;
        }

        log.info("Handling record: {}", value.get());

        // TODO: Here is where you can handle your records
    }

    @Bean
    public void executeKafkaCommentsRunner() {
        new SimpleAsyncTaskExecutor().execute(this);
    }
}
