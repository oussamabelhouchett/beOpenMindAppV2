...
public class ApplicationUserResourceIT {
    ...
    @Test
    @Transactional
    public void getAllApplicationUsersByPostIsEqualToSomething() throws Exception {
        // Initialize the database
        applicationUserRepository.saveAndFlush(applicationUser);
        Post post = PostResourceIT.createEntity(em);
        em.persist(post);
        em.flush();
        applicationUser.setPost(post);
        applicationUserRepository.saveAndFlush(applicationUser);
        Long postId = post.getId();

        // Get all the applicationUserList where post equals to postId
        defaultApplicationUserShouldBeFound("postId.equals=" + postId);

        // Get all the applicationUserList where post equals to postId + 1
        defaultApplicationUserShouldNotBeFound("postId.equals=" + (postId + 1));
    }


    @Test
    @Transactional
    public void getAllApplicationUsersByCommentsIsEqualToSomething() throws Exception {
        // Initialize the database
        applicationUserRepository.saveAndFlush(applicationUser);
        Comments comments = CommentsResourceIT.createEntity(em);
        em.persist(comments);
        em.flush();
        applicationUser.setComments(comments);
        applicationUserRepository.saveAndFlush(applicationUser);
        Long commentsId = comments.getId();

        // Get all the applicationUserList where comments equals to commentsId
        defaultApplicationUserShouldBeFound("commentsId.equals=" + commentsId);

        // Get all the applicationUserList where comments equals to commentsId + 1
        defaultApplicationUserShouldNotBeFound("commentsId.equals=" + (commentsId + 1));
    }


    @Test
    @Transactional
    public void getAllApplicationUsersByReactionIsEqualToSomething() throws Exception {
        // Initialize the database
        applicationUserRepository.saveAndFlush(applicationUser);
        Reaction reaction = ReactionResourceIT.createEntity(em);
        em.persist(reaction);
        em.flush();
        applicationUser.setReaction(reaction);
        applicationUserRepository.saveAndFlush(applicationUser);
        Long reactionId = reaction.getId();

        // Get all the applicationUserList where reaction equals to reactionId
        defaultApplicationUserShouldBeFound("reactionId.equals=" + reactionId);

        // Get all the applicationUserList where reaction equals to reactionId + 1
        defaultApplicationUserShouldNotBeFound("reactionId.equals=" + (reactionId + 1));
    }
